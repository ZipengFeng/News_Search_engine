...
class Doc:
    ...
    ld = 0
    comments_num = 0
    def __init__(self, docid, date_time, tf, ld,comments_num):
        self.docid = docid
    ...
    def __repr__(self):
        return(str(self.docid) + '\t' + self.date_time + '\t' + str(self.tf) + '\t' + str(self.ld) + '\t' + str(self.comments_num))
    def __str__(self):
        return(str(self.docid) + '\t' + self.date_time + '\t' + str(self.tf) + '\t' + str(self.ld) + '\t' + str(self.comments_num))
...
class IndexModule:
    ...
    def construct_postings_lists(self):
        ...
        files = listdir(config['DEFAULT']['sorted_doc_dir_path'])
        AVG_L = 0
        for i in files:
            root = ET.parse(config['DEFAULT']['sorted_doc_dir_path'] + i).getroot()
            ...
            for key, value in cleaned_dict.items():
                d = Doc(docid, date_time, value, ld,comments_num)
