...
class RecommendationModule:
    ...
    def __init__(self, config_path, config_encoding):
        ...
        self.doc_dir_path = config['DEFAULT']['sorted_doc_dir_path']
    ...
    def construct_dt_matrix(self, files, topK = 200):
        ...
        for i in files:
            ...
            try:
                tags = jieba.analyse.extract_tags(title + u'。' + body, topK=topK, withWeight=True)
            except:
    ...
    def gen_idf_file(self):
        ...
        for i in files:
            ...
            body = root.find('body').text
            if count % 100 == 0:
                print "1K done." + str(count)
            count += 1
            try:
                seg_list = jieba.lcut(title + u'。' + body, cut_all=False)
            except:
                print("Segamentation Error.")
                continue
            ...
            for word in seg_list:
                ...
                if word not in idf:
                    ...
                    idf[word] = idf[word] + 1
        idf_file = open(self.idf_path, 'wb')
        for word, df in idf.items():
            idf_file.write((u'%s %.9f\n'%(word, math.log(n / df))).encode('utf-8'))
        idf_file.close()
